<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:68:"/www/wwwroot/app.yuxiaxie888.cn/application/index/view/user/add.html";i:1538726159;s:66:"/www/wwwroot/app.yuxiaxie888.cn/application/index/view/layout.html";i:1538650796;}*/ ?>
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="utf-8">
        ...
        <form role="form" id="form1" method="post" action="<?php echo url('user/add'); ?>" class="validate">
            ...
            <div class="form-group">
                ...
                <select name="power" id="ss"  class="form-control">
                    <option value="1" onclick="dai()" <?php echo !empty($data['power']) && $data['power']=='1'?'selected':''; ?>>开通代理&nbsp;(需扣除五十点)</option>

                </select>

            </div><?php }if(session('power')=='0'){?>
            <div class="form-group">
                <label class="control-label">分享码</label>
                <input type="text" class="form-control" name="share_ma"  placeholder="分享码码不能为汉字（可留空即随机获取分享码）" />
            </div><?php }?>
